    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        ...
        if (sessionName.isBlank()) {
            ...
            Row(Modifier.fillMaxWidth().padding(top=10.dp), horizontalArrangement=Arrangement.spacedBy(8.dp)) {
                ...
                OutlinedButton(enabled=!loading, onClick={ run { val pair=repo.notificationHistory(); historyText="Gönderilen: ${pair.first.size} • Zamanlanan: ${pair.second.count{it.status=="SCHEDULED"}}\n" + pair.first.take(8).joinToString("\n") { "${it.topic} — ${it.title}" } } }, modifier=Modifier.weight(1f)) { Icon(Icons.Default.History,null); Spacer(Modifier.width(6.dp)); Text("Geçmiş") }
            }
            OutlinedButton(enabled=!loading, onClick={ run { val st=repo.notificationStats(); historyText="Toplam gönderim: ${st.totalSent}\nZamanlanan: ${st.totalScheduled}\nTaslak: ${st.totalDrafts}\nKonu dağılımı: ${st.byTopic}" } }, modifier=Modifier.fillMaxWidth().padding(top=8.dp)){Text("Bildirim istatistikleri")}
